...
Route::middleware('guest')->group(function () {
	...
	Route::view('/new-password/{userId}/{token}', 'password.update-page')->middleware(['checkVerified', 'verifyToken'])->name('passwords.update_page');
});
...
Route::controller(ResetPasswordController::class)->middleware(['guest', 'checkVerified'])->group(function () {
	...
	Route::patch('/new-password/{userId}/{token}', 'updatePassword')->middleware('verifyToken')->name('passwords.update');
});
